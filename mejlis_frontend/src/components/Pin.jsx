...
const Pin = ({ pin : { postedBy, image, _id, save, userID, destination}}) => {
  ...
  const decode_id= jwt_decode(user.credential);
  const the_Id= decode_id.sub;
  const alreadySaved = !!(save?.filter((item) => item.postedBy?._id === the_Id));
